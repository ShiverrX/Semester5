...
def filter_transactions_by_product(transactions, product_name):
    filtered_transactions = filter(lambda transaction: transaction["product"] == product_name, transactions)
...
filtered_transactions = filter_transactions_by_product(transactions, product_filter_input)

# Menggunakan map() untuk menghitung total harga untuk setiap transaksi yang tersaring
transaction_totals = list(map(calculate_transaction_total, filtered_transactions))
...
print("Total Jumlah Item Terjual dari Produk", product_filter_input, ":", sum([transaction["quantity"] for transaction in filtered_transactions]))
